...
==================================
Servidor local (Flask) que recibe datos en vivo del script de reconocimiento
de gestos (gestureRecognition.py -> metrics_client.py) y los guarda en una
base de datos SQLite. Sirve además la página web (static/index.html) que
grafica los movimientos de la mano y las métricas del proyecto:
  ...
  - Vista en vivo del estado de los 5 dedos
...
DB_PATH = os.path.join(BASE_DIR, "sianna.db")
...
def init_db():
    ...
    # Migra bases de datos creadas antes de agregar el desglose de latencia por etapa.
    for col in ("captura_ms", "mediapipe_ms", "inferencia_ms", "serial_ms", "servo_ms"):
        _ensure_column(conn, "gesture_events", col, "REAL")
    conn.commit()
    conn.close()
...
if __name__ == "__main__":
    init_db()
    print("SIANNA web -> http://127.0.0.1:5050")
